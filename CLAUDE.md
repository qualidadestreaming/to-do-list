...
- [ ] **Fase 5 — Migração de dados**: planilha `TDL QA MULTI 2026.xlsx` já foi inspecionada (ver seção própria abaixo) — script de importação ainda não escrito.
- [ ] **Fase 6 — Administração e expansão**: painel de gestor (`/app/admin`) ainda é um stub estático.

## A planilha original (`TDL QA MULTI 2026.xlsx`) — achados para a Fase 5

Usuário enviou o arquivo em `C:\Users\israe\Downloads\TDL QA MULTI 2026.xlsx` (caminho local dele, não está no repo). Inspecionado com a lib `xlsx` (SheetJS — **tem 1 vulnerabilidade "high" conhecida no npm audit**, mas só é usada como devDependency num script de migração one-off, nunca em código de produção/browser, risco aceito).
